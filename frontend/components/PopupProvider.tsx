      ...
      <div className="fixed top-20 right-4 z-[100] space-y-3">
        {toasts.map((toast) => {
          const styleMap: Record<ToastType, string> = {
            success: 'bg-sky-500/20 border-sky-500/40 text-sky-200',
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2">
                {toast.type === 'success' && <CheckCircle2 size={18} className="mt-0.5 text-sky-300" />}
                {toast.type === 'error' && <XCircle size={18} className="mt-0.5 text-red-300" />}
                {toast.type === 'info' && <AlertTriangle size={18} className="mt-0.5 text-slate-300" />}
      ...
      {confirmState && (
        <div className="fixed inset-0 z-[110] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl">
            ...
            <p className="text-slate-300 mt-2">{confirmState.message}</p>
            <div className="mt-6 flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-red-500 hover:bg-red-600 text-slate-950 font-semibold rounded-lg transition-colors"
              >
                Delete
              </button>
            </div>
